...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210628041650_InitialCreate')
BEGIN
...
BEGIN
    ...
    VALUES (''47d0fc63-ccd3-48c6-825c-496a56db4adb'', N''Figuran ejecutivas de D1, Cencosud, Cerrejón y Comparta. Tres de ellas asumieron en los últimos dos años'', N''Solo hay 4 mujeres en los CEO de las 100 empresas más grandes''),
    (''056cff93-297c-4145-8035-ded5cbc32c61'', N''El préstamo, incluye recursos para la construcción del Parque Solar San Fernando (Meta) por cerca de US$35 millones'', N''Crédito de Scotiabank Colpatria por US$100 millones a AES Colombia''),
    (''9e77ccaf-98f6-426f-9f95-df113d985235'', N''Se trata de Shopify, que brindará soporte y apoyo a comerciantes que no usan directamente su plataforma'', N''Google y Facebook firman asociación con empresa de pago en línea''),
    (''b0ec35d4-9a19-4e6d-8cd2-817b968c2bb8'', N''Los delincuentes envían cartas a las viviendas de sus víctimas haciéndose pasar por la compañía Ledger. En el documento piden a las personas compartir su información en un nuevo dispositivo para supuestamente mantener los datos seguros'', N''Bitcóin: hackers tienen nuevo método para robar criptomonedas''),
    (''a6e7bad2-ac9c-407a-a0bb-2d32b51e794a'', N''Habi, la startup con sede en Colombia, anunció este miércoles que recaudó 100 millones de dólares en la ronda de financiación de serie B por parte de SoftBank Latin American Fund, junto con inversores de Inspired Capital, Tiger Global, Homebrew y 8VC'', N''''''Startup'''' Habi recauda 100 millones de dólares en ronda de inversión''),
    (''c2cca9ea-b8c1-4abf-b70c-e47be5f06f92'', N''Después de un año de ausencia por la pandemia, finalmente el Electronic Entertainment Expo (E3) volvió con varias novedades para los amantes de los videojuegos, aunque no cumplió todas las expectativas en materia de los anuncios que se tenían presupuestados'', N''E3: los mejores anuncios que dejó el evento en su edición virtual'')');
    IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Description', N'Title') AND [object_id] = OBJECT_ID(N'[dbo].[News]'))
...
BEGIN
    ...
    VALUES (''c651f92d-d9a1-45e8-9d04-3bb184da7a96'', ''Administrador'', ''AdminUser''),
    (''f7b2beba-7445-463d-8978-0b3b3940390c'', ''Asistente'', ''AssistantUser''),
    (''590cb67a-5177-4bda-86e4-e5b80ac333c9'', ''Visitante'', ''VisitorUser''),
    (''3cb57f96-7092-4842-a3a4-5e8302f6618d'', ''Editor'', ''EditorUser'')');
    IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'DisplayName', N'Name') AND [object_id] = OBJECT_ID(N'[dbo].[Role]'))
...
BEGIN
    ...
    VALUES (''4f39c31e-ca99-4782-b585-ae847185a292'', ''590cb67a-5177-4bda-86e4-e5b80ac333c9''),
    ...
    (''4f39c31e-ca99-4782-b585-ae847185a292'', ''c651f92d-d9a1-45e8-9d04-3bb184da7a96''),
...
BEGIN
    ...
    VALUES (N'20210628041650_InitialCreate', N'5.0.7');
END;
GO
